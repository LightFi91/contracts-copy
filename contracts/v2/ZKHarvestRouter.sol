...
contract ZKHRouter is IZKHRouter {
    ...
    function addLiquidity(
        ...
        address to,
        uint deadline
    ) public ensureNotExpired(deadline) returns (uint amountA, uint amountB, uint liquidity) {
        ...
        (uint reserveA, uint reserveB) = ZKHLibrary.getReserves(factory, tokenA, tokenB);
    ...
    function _calculateAmountsToAdd(LiquidityData memory data) internal pure returns (uint amountA, uint amountB) {
        if (data.reserveA == 0 && data.reserveB == 0) {
            return (data.amountADesired, data.amountBDesired);
        }
        uint amountBOptimal = ZKHLibrary.quote(data.amountADesired, data.reserveA, data.reserveB);
        if (amountBOptimal <= data.amountBDesired) {
            return (data.amountADesired, amountBOptimal);
        } else {
            uint amountAOptimal = ZKHLibrary.quote(data.amountBDesired, data.reserveB, data.reserveA);
    ...
    function _transferTokens(address tokenA, address tokenB, uint amountA, uint amountB, address to) internal {
    ...
    function swapExactTokensForTokensWithFee(
        ...
        amounts = ZKHLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "ZKHRouter: INSUFFICIENT_OUTPUT_AMOUNT");
        
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ZKHLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i = 0; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            IZKHarvestPair pair = IZKHarvestPair(ZKHLibrary.pairFor(factory, input, output));
            uint amountInput = amounts[i];
            uint amountOutputExpected = amounts[i + 1];
            uint amountOutputActual;
            {
                (uint reserveInput, uint reserveOutput) = ZKHLibrary.getReserves(factory, input, output);
                amountOutputActual = ZKHLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            require(amountOutputActual >= amountOutputExpected, "ZKHRouter: INSUFFICIENT_OUTPUT_AMOUNT");
            (uint amount0Out, uint amount1Out) = input == pair.token0() ? (uint(0), amountOutputActual) : (amountOutputActual, uint(0));
